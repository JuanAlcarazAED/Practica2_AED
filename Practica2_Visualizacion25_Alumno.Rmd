...
```{r}
# Lo primero es borrar el espacio de variables
rm(list = ls())

# Ahora usando la función load y rutas locales guardamos
load("./data/weatherAUS.RData")
# Vemos que se llama ds la nueva variable donde se guardan los datos
```
...
```{r}
# Para crear un gráfico de deispersión es necesario es necesario poner puntos
...
```{r}
grafico3 <- grafico2 + ggtitle("Datos de temperatura máxima y mínima para días lluviosos y no lluviosos")
grafico3
```
...
* Utilizando ggplot haz un gráfico de dispersión en el que representes las variables ``Temp9am`` y ``Pressure9am`` en los ejes $x$ e $y$ respectivamente.
...
```{r}
grafico4 <- ggplot(ds, aes(x = Temp9am, y = Pressure9am, shape = RainToday)) + geom_point()
grafico4
```
...
```{r}
grafico4 <- ggplot(ds, aes(x = Temp9am, y = Pressure9am, color = RainToday)) + geom_point()
grafico4
```
La información se observa mucho mejor con `color`.
...
```{r}
grafico5 <- grafico4 + labs(title = "Temperatura y presión a las 9 am",
     subtitle = "Relación entre la tempretura y presión de un día con las precipitaciones",
     x = "Temperatura a las 9:00 (en grados centígrados)",
     y = "Presión atmosférica a las 9:00 (en bares)",
     caption = "Datos obtenidos de mediciones manuales",
     colour='Día lluevioso')
grafico5
```
...
* Carga el fichero ``iris_tidy.Rdata`` utilizando lo aprendido en la práctica 1.

```{r}
# Como vamos a cambiar el conjunto de datos reiniciamos el espacio de variables
rm(list = ls())

load("./data/iris_tidy.Rdata")
# Vemos que la nueva variable se llama iris.tidy
```
...
```{r}
# Como vamos a cambiar de conjunto de datos limpiamos las variables
rm(list = ls())
...
```{r}
grafico1 <- ggplot(DatosEjer4, aes(x = Fecha, y = Ventas, color = Fabricante)) + geom_point()
grafico1
```
El gráfico no es muy claro, aunque los colores ayudan a distinguir los fabricantes, las nuebes de puntos están inconexas y se mezclan.
...
```{r}
grafico3 <- grafico1 + geom_line(aes(linetype = Fabricante))
grafico3
```
* Añade el siguiente título al gráfico *"Ventas mensuales de diferentes compañías tecnológicas"*.

```{r}
grafico3 <- grafico3 + labs(title = "Ventas mensuales de diferentes compañías tecnológicas")
grafico3
```
...
```{r}
# Antes de comenzar limpiamos el espacio de
rm(list = ls())
...
```{r}
# Primero hay que limpiar el espacio de variables
rm(list = ls())
...
```{r}
grafico1 <- ggplot(data = Experimento, aes(x = Dosis, y = Altura, linetype = Complemento, shape =                    Complemento)) + 
            geom_point(aes(color = Complemento)) + geom_line(aes(color = Complemento))
grafico1
...
```{r}
grafico2 <- grafico1 +
            geom_errorbar(aes(ymin=(Altura - Error_Altura), ymax=(Altura + Error_Altura)), width=0.1,                position=position_dodge(0.05))
grafico2
```
...
```{r}
grafico3 <- ggplot(data = Experimento, aes(x = Dosis, y = Altura)) + 
            geom_point(aes(color = Complemento, shape = Complemento)) + 
            geom_line(aes(color = Complemento, linetype = Complemento)) + 
            geom_errorbar(aes(ymin=(Altura - Error_Altura), ymax=(Altura + Error_Altura)), width=0.1,                position=position_dodge(0.05))

grafico3 <- grafico3 + scale_color_brewer(palette="Reds")
...
```{r}
# Reseteamos el espacio de las variables
rm(list = ls())
...
# Ahora representamos los caudales respecto del tiempo
grafico1 <- ggplot(ice.river.df, aes(x = time)) + 
            geom_line(mapping = aes(y = flow.vat, color = "red")) + 
            geom_line(mapping = aes(y = flow.jok, color = "blue"))
grafico1
```
...
```{r}
grafico1 <- grafico1 + labs(title = "Serie de tiempo",
                 subtitle = "Flujo de agua diario en los ríos Vatnsdalsa y Hveravellir",
                 x = "Flujo",
                 y = "Tiempo",
                 caption = "Flujo díario en los ríos",
                 colour='Río')
grafico1
```
...
```{r}
# Eliminamos las variables previas
rm(list = ls())
...
```{r}
grafico1 <- ggplot(data = ds, aes(x = Year, y = MinTemp)) +
            geom_boxplot()
...
```{r}
grafico2 <- ggplot(data = ds, aes(x = Year, y = MaxTemp)) +
...
```{r}
grafico2 <- grafico2 + theme(legend.position = "none") + 
...
```{r}
grafico1 <- grafico1 + labs(title = "Histograma de la variable altura",
                              x = "Altura (cm)",
                              y = "Frecuencia")
grafico1
```
...
```{r}
grafico2 <- ggplot(data = chol, aes(x = HEIGHT)) + 
            geom_histogram(breaks=seq(155, 195, by=(195 - 155) / 20))
grafico2
```
...
```{r}
grafico3 <- ggplot(data = chol, aes(x = HEIGHT)) + 
            geom_histogram(breaks=seq(155, 195, by=(195 - 155) / 20),
                           fill = "blue",
                           color = "red",
                           alpha = 0.5)
...
```{r}
grafico4 <- ggplot(data = chol, aes(x = HEIGHT)) + 
            geom_histogram(aes(y = after_stat(density)),
                           breaks=seq(155, 195, by=(195 - 155) / 20),
                           fill = "blue",
                           color = "red",
                           alpha = 0.5) + geom_density(col="green")
grafico4
```
...
```{r}
grafico5 <- ggplot(data = chol, aes(x = HEIGHT)) + 
            geom_histogram(aes(y = after_stat(density)),
                           breaks=seq(155, 195, by=(195 - 155) / 20),
                           fill = "blue",
                           color = "red",
